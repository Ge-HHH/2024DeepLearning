...
class LSTMClassifier(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Conv2d(128,16,kernel_size=3,stride=1)
        self.conv2 = nn.Conv2d(16,4,kernel_size=3,stride=1)
        # self.conv1 = nn.Conv2d(3,16,kernel_size=4,stride=4)

    def forward(self, x):
        # x=self.conv1(x)
        # x = x.view(-1, 64,16)  # Reshape input to (batch_size, seq_len, input_size)
        x = self.patch_embed(x)
        lstm_out, _ = self.lstm(x)
        x = lstm_out.view(-1,8,8,128)
        x = x.permute(0,3,1,2)
        #[8,8,128]
        x = nn.ReLU()(self.conv1(x)) #[6,6,16]
        x = nn.ReLU()(self.conv2(x)) #[4,4,4]
        x = x.view(-1,4*4*4)
...
if __name__=="__main__":
    ...
    model.train(num_epochs=5, batch_size=256,lr=2e-3,weight_decay=0.001,dropout=0.2)
    model.test()
    # 保存模型
